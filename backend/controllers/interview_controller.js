...
module.exports.schedule = async function (req, res)
{
    ...
    {
        console.log("*****************************");
        const interviewer = req.body.email.split(',');
        ...
        if (y >= cy && m >= cm && d >= cd)
            if (y == cy && m == cm && d == cd) {
                if (st_hr * 60 + st_min >= current_hour * 60 + current_min && en_hr * 60 + en_min > st_hr * 60 + st_min)
                    ...
                    return res.status(404).json({ mess: "Interview cannot be shedule" });
            }
            else
            {
                console.log("interview sechdule")
    ...
    {
        console.log(err);
        return res.status(404).json({ mess: "interview schedule not created" });
    }
}
